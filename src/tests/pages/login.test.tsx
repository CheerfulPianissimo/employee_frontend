...
import { Login } from "../../pages/login"; // Mock react-router-dom's useNavigate
...
describe("Login Page", () => {
  ...
  it("should show error when username is too long", async () => {
    ...
    await userEvent.type(usernameInput, "a".repeat(21)); // Check if error message is displayed
    expect(
      screen.getByText("Username cannot be longer than 20 chars.")
    ).toBeInTheDocument();
  });
  ...
  it("should handle login failure", async () => {
    mockLoginMutation.mockRejectedValueOnce({ data: {message:"Invalid"} });
